package com.uplink.carins.ui.city;
...
public class ScrollerNumberPicker extends View {
    ...
    public void setDefault(int index) {
        ...
        LogUtil.e("选择中的:" + item.itemText);
        float move = item.moveToSelected();
        //actionUp((int) move);
    ...
    public void setDefaultByName(String name) {
        ...
        for (int i = 0; i < itemList.size(); i++) {
            ...
            if (itemList.get(i).itemText.indexOf(name) > -1)

                postion = i;

        }
        ItemObject item = itemList.get(postion);
        ...
        if (onSelectListener != null)
            onSelectListener.endSelect(itemList.get(postion).id,
                    itemList.get(postion).itemText);
